  ...
  try {
    ...
    if (orderError) {
      throw orderError;
        ...
        products.map( async (product: { id: number, quantity: number, price: number }) => {
          // Decrease Inventory
          const decreaseStock = await supabase.rpc('decrement_stock', { x: product.quantity, pid: product.id })
          return product
        });
